...
const RegisterForm = (props) => {
  ...
  function prepareFormData(form) {
    const position_id = positions.find(
      (position) => position.name === form.position.value
    ).id;
  ...
  async function handleSubmit(event) {
    ...
    if (validation.success) {
      ...
      try {
        ...
        setRegisterStatus('unsuccessful');
      } finally {
        setIsLoading(false);
      }
    } else {
    ...
    <>
      ...
      >
        <div className='register-form__text-inputs'>
          <div className='register-form__text-input-wrapper'>
            <input
              className={
                validationErrors.username
                  ? 'register-form__text-input border-error'
            ...
            <label
              className={
                validationErrors.username
                  ? 'register-form__text-input-label label-error'
            ...
            {validationErrors.username && (
              <small className='register-form__error-text'>
                {validationErrors.username}
              </small>
            )}
          </div>
          ...
          <div className='register-form__text-input-wrapper'>
            ...
            {validationErrors.phone ? (
              ...
              <small className='register-form__helper-text'>
                +38 (XXX) XXX - XX - XX
              </small>
            )}
          </div>
        ...
        <fieldset className='register-form__radio-inputs'>
          <legend className='register-form__radio-inputs-title'>
          ...
          {positions &&
            positions.map((item, index) => (
              <div className='register-form__radio-input-wrapper' key={index}>
                ...
                />
                <label
                  className='register-form__radio-input-label'
                  htmlFor={item.id}
                >
                  {item.name}
                </label>
        ...
        <div
          className={
            validationErrors.photo
              ? 'register-form__upload-inputs-wrapper border-error'
              : 'register-form__upload-inputs-wrapper'
          }
        >
          <label
            className={
              validationErrors.photo
                ? 'register-form__upload-input-label border-error'
                : 'register-form__upload-input-label'
            }
            htmlFor='file-upload'
          >
            Upload
          ...
          <span
            className={
              fileName === 'Upload your photo'
                ? 'register-form__upload-placeholder'
                : 'register-form__upload-placeholder active-placeholder'
            }
          >
            {fileName}
          </span>
